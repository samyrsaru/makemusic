...
// Download generation audio file (for mobile compatibility)
app.get('/:id/download', async (c) => {
  ...
  // Get signed URL if R2 key exists
  let audioUrl = generation.audioUrl
  if (generation.r2Key) {
    try {
      audioUrl = await getSignedAudioUrl(generation.r2Key, 300) // 5 minutes
    } catch (err) {
      console.error(`Failed to generate signed URL for download ${id}:`, err)
    }
  }
  ...
  try {
    ...
    c.header('Content-Disposition', `attachment; filename="song-${id}.mp3"`)
    c.header('Cache-Control', 'no-cache')
    
    return c.body(arrayBuffer)
  } catch (err) {
    console.error(`Download failed for generation ${id}:`, err)
